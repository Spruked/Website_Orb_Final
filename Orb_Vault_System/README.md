...
┌─────────────────────────────────────────────────────────┐
│                    ORB Assistant                         │
│              (VaultCoordinator + QueryRouter)           │
├─────────────────────────────────────────────────────────┤
│  A Priori Vault          │  A Posteriori Vault          │
│  ─────────────────        │  ─────────────────────        │
│  • Catalog (products,     │  • Experience Ledger          │
│    prices, SKUs)         │    (immutable, append-only)   │
│  • Ontology (business    │  • Candidate Queue            │
│    understanding graph)  │  • Verification Engine        │
│  • QA Correspondences    │  • Promotion Gate             │
│  • Owner Policies        │  • Reinforcement Engine       │
│                          │  • Contradiction Detector     │
│  Source: Orb Weaver      │  • Self-Pruning Engine        │
│  (crawl/owner-approved)  │  • Merge/Compress             │
│                          │                               │
│  No learning.            │  Self-improving.              │
│  Direct lookup only.     │  Self-pruning.                │
│                          │  Bounded confidence.          │
└─────────────────────────────────────────────────────────┘
```
...
```bash
# Copy vault/ into your ORB Assistant project
cp -r orb_vault/vault/ /path/to/orb_assistant/

# Create Orb Weaver output directory structure
mkdir -p /path/to/weaver_output
# Place catalog.json, ontology.json, qa.json, policies.json there
